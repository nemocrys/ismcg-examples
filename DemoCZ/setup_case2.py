...
def simulation_pyelmer(model, config, sim_dir="./simdata", config_mat={}, elmer_config_file="config_elmer.yml"):
    ...
    crystal.equation = eqn_heat_stress # The stress solver is applied on the crystal
    ...
    bnd.heat_transfer_coefficient = config["boundaries"]["bnd_melt"]["htc"] # Assign Heat Transfer Coefficient
    ...
    bnd.radiation_idealized =True    
    bnd.data.update({"Radiation External Temperature" : config["T_ambient"]}) 
    bnd.data.update({"Displacement 2":0.0})  # Necessary for Stress solver
    bnd.save_scalars = True
