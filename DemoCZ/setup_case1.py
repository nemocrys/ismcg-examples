...
from geometry_case1 import geometry


def simulation_pyelmer(model, config, sim_dir="./simdata", config_mat={}, elmer_config_file="config_elmer.yml"):


    ###--- In the first part, we load the solvers from the elmer.yml file and then combine them to form different equations, 
    # which will subsequently be applied to simulation bodies. ---###
    ...
    # eqn_flow_melt.data.update({"Convection" : "Computed" }) # Case 2
    # eqn_flow_gas = elmer.Equation(sim, "eqn_flow_gas", [solver_mgdyn, solver_calcfields , solver_heat , solver_Navier_Stokes ]) # Case 2
    # eqn_flow_gas.data.update({"Convection" : "Computed" }) # Case 2

    # FORCES SETUP
    #Induction heating 22kHz, 30Ax20windings: 600A/(0.07*0.005) = 1.7e6   # CASE 2,3
    ...
    hotplate_force.smart_heat_control  = True #This line force the simulation to converge, once the condition below fulfilled(i.e 505K at coordinate[0.005,0.01,0]) 
    ...
    # Induction heating part # CASE 2,3
    # induction = elmer.Body(sim,'EM_coil', [model["EM_coil"].ph_id]) # We have to mention the body name from the model we constructed at from geometry.py
    ...
    mat = elmer.Material(sim, material_name, config_mat[material_name])
    melting_point = mat.data["Melting Point"] # set to 505 K
    ...
    #crystal.body_force = joule_heat Case 2,3
    ...
    #melt.body_force = joule_heat # Case 3
    ...
    #atmosphere.body_force = joule_heat # Case 3
    #atmosphere.body_force = gas_flow # Case 2


    ###--- In the final section, Boundary Conditions are defined. 
    # Each Boundary must contain variable information essential for the equations-solvers that we are solving within the simulation domain.---###
    ...
    # Crystal top Boundary Condition - Stress solver 
    bnd = elmer.Boundary(sim, "bnd_crystal_top", [model["bnd_crystal_top"].ph_id])
    bnd.radiation_idealized =True    
    bnd.data.update({"Radiation External Temperature" : config["T_ambient"]}) 
    # bnd.data.update({"Velocity 1" : 0.0}) # Case 2
    # bnd.data.update({"Velocity 2" : 0.0}) # Case 2
    # bnd.data.update({"Displacement 2":0.0})  #  Case 2
    bnd.save_scalars = True

    # Magnet Boundary Condition
